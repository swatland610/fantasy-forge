...
--     (JAC vs JAX, LAR vs LA), which a hard filter would also break.
...
--     analyst_name_aliases seed, applied BEFORE normalization below
...
-- the analyst text? Cross-checked all 60 team_mismatch rows against Sleeper's public
-- player API (independent of nflverse) by gsis_id (13 rows) and by name for the rest
-- (43 rows; mostly 2025/26 rookies Sleeper hasn't yet linked a gsis_id for) -- 56/60
-- confirmed exactly, the remaining 4 confirmed by relaxing suffix punctuation in the
-- name match. Zero real disagreements. So `team` (exposed below, sourced from
-- dim_players/roster_team) is the trustworthy one; analyst_stated_team is kept only
-- for audit/provenance and should NOT be used downstream -- team_mismatch marks where
-- the two disagree, which in every checked case means the ANALYST text was wrong
-- (stale team, transcription slip, or a JAC/JAX-, LAR/LA-style spelling difference),
-- not dim_players.
...
deduped as (
    ...
    -- position pair resolves to more than one player_id, but never require it.
    select *
    from candidates
    qualify row_number() over (
        partition by analyst, player_name, position, team, source_url
        order by (team = roster_team) desc, player_id
    ) = 1
...
select
    ...
    (player_id is not null and team is distinct from roster_team) as team_mismatch,
    overall_rank,
    position_rank,
    tier,
    source_url,
